...
if [[ -n "$schema" ]]; then
  ...
  output_file="./${safe_dbname}${separator}${safe_schema}.backup.gz"
else
  output_file="./${safe_dbname}.backup.gz"
fi
...
pg_args=(pg_dump --verbose --lock-wait-timeout=60 -h "$host" -p "$port" -U "$username" -d "$dbname")
if [[ -n "$schema" ]]; then
  pg_args+=(-n "$schema")
fi

mapfile -t docker_host_args < <(docker_host_args_for "$host")
time docker run --rm "${docker_host_args[@]}" "${env_args[@]}" postgres:17 "${pg_args[@]}" | gzip -9 >"$output_file"
